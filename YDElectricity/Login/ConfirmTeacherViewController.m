...
- (IBAction)OnOwnTeacherBtnClick:(id)sender {
    ...
    if (self.OwnSelectTeacherTextField.text.length < 1 || [self.OwnSelectTeacherTextField.text isEqualToString:@" "]) {
        ...
        //请求注册
        [weakSelf.registerVM requestRegisterWithInviteCode:self.OwnSelectTeacherTextField.text wxOpenID:[YDUserInfo sharedYDUserInfo].userWxOpenID userPhoneNum:[YDUserInfo sharedYDUserInfo].phoneNumber CompletionHandler:^(NSError * _Nonnull error) {
            if (!error) {
                [weakSelf.view endEditing:YES];//强行关闭键盘
                [weakSelf.view showWarning:@"注册成功"];
                NSString *wxOpenID = [YDUserInfo sharedYDUserInfo].userWxOpenID;
                NSString *phoneNum = [YDUserInfo sharedYDUserInfo].phoneNumber;
                NSLog(@"wxOpenID:%@,phoneNum:%@",wxOpenID,phoneNum);
