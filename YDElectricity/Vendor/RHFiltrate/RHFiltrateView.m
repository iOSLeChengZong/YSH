...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    float width = [RHHelper getWidthByText:_itemArr[indexPath.section][indexPath.row] font:[UIFont systemFontOfSize:12]] + ItemWidth;
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout referenceSizeForFooterInSection:(NSInteger)section {
    ...
    return CGSizeZero;//CGSizeMake(self.bounds.size.width, 1);
}
...
- (void)hideMaskView
{
    [UIView animateWithDuration:0.25 animations:^{
        ...
        CGRect frame = self.self.collectionView.frame;
        frame.size.height = 0;
        self.self.collectionView.frame = frame;
    }completion:^(BOOL finished) {
        [self.self.collectionView removeFromSuperview];
